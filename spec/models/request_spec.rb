...
RSpec.describe Request, type: :model do
  ...
  describe "#determine_required_fields?" do
    it "default list" do
      expect(subject.determine_required_fields).to match_array(%w[referral_type description is_nc ee_assess_required character_desc])
    end
    it "when tax subsidy" do
      subject = described_class.new(scheme_subsidy: "subsidy", subsidy_form: "tax")
      expect(subject.determine_required_fields).to match_array(%w[referral_type description is_nc ee_assess_required subsidy_form ben_id_type ben_id beneficiary ben_size legal policy confirm_date character_desc tax_amt])
    end
    it "when loan subsidy" do
      subject = described_class.new(scheme_subsidy: "subsidy", subsidy_form: "loan", ee_assess_required: "n")
      expect(subject.determine_required_fields).to match_array(%w[referral_type description is_nc ee_assess_required subsidy_form ben_id_type ben_id beneficiary ben_size legal policy confirm_date character_desc budget])
    end
    ...
    it "when scheme" do
      ...
      expect(subject.determine_required_fields).to match_array(%w[scheme_name legal policy confirm_date start_date referral_type description is_nc ee_assess_required character_desc])
    end
    ...
    it "when scheme call in" do
      ...
      expect(subject.determine_required_fields).to match_array(%w[scheme_name legal policy confirm_date start_date referral_type direction_date call_in_type description is_nc ee_assess_required character_desc])
    end
    it "when scheme PAR not on TD" do
      subject = described_class.new(referral_type: "par", scheme_subsidy: "scheme", par_on_td: "n")
      expect(subject.determine_required_fields).to match_array(%w[referral_type description is_nc direction_date par_on_td par_assessed scheme_name legal policy confirm_date start_date ee_assess_required])
    end
    it "when scheme PAR on TD accessed" do
      ...
      expect(subject.determine_required_fields).to match_array(%w[referral_type description is_nc direction_date par_on_td par_td_ref_no par_assessed ee_assess_required])
    end
